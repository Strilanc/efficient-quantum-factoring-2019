...
\\
&&\lstick{|+\rangle} &\gate{R_z(...)} &       \multigate{3}{\text{Input }e_{0:4}}&       \multigate{3}{\text{QFT}}&\meter&&&&\lstick{|+\rangle} &\gate{R_z(...)} &       \multigate{3}{\text{Input }e_{4:8}}&       \multigate{3}{\text{QFT}}&\meter&&&\dots\\
&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{0:4}}&              \ghost{\text{QFT}}&\meter&&&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{4:8}}&              \ghost{\text{QFT}}&\meter&&&\dots\\
&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{0:4}}&              \ghost{\text{QFT}}&\meter&&&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{4:8}}&              \ghost{\text{QFT}}&\meter&&&\dots\\
&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{0:4}}&              \ghost{\text{QFT}}&\meter&&&&\lstick{|+\rangle} &\gate{R_z(...)} &              \ghost{\text{Input }e_{4:8}}&              \ghost{\text{QFT}}&\meter&&&\dots\\
&{/}\qw&\ustick{n}\qw&\qw             &\gate{\times \gen^{2^0 e_{0:4}} \pmod{N}}\qwx&                      \qw&   \qw&\qw&\qw&\qw&\qw       &\qw             &\gate{\times \gen^{2^4 e_{4:8}} \pmod{N}}\qwx&                      \qw&\qw&\qw&\qw&\dots\\
\\
}
}
    \caption{
    \label{fig:exponent-grouping}
    Working through the qubits representing an exponent in Shor's algorithm with a window size $\gexp$ of four, while using a semi-classical Fourier transform \cite{griffiths1996semiclassical}.
    Each $R_z(\dots)$ gate rotates by an amount determined by previous measurements.
    All 16 possible values of the expression $\gen^{e_{4:8} 2^4} \pmod{N}$ (and similar expressions) can be precomputed classically, and looked up on demand within the multiplication circuit.
